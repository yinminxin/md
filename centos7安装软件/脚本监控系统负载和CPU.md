...
```shell
echo "测试邮件内容123456" | mailx -v -s "测试标题" -a "附件全路径" yinminxin@xxx.com
```
...
　　echo "$IP服务器15分钟的系统平均负载为$average_load，超过警戒值1.0，请立即处理！！！" | mailx -v -s "$IP 服务器系统负载严重告警！！！" yinminxin@xxx.com 
　　else 
...
　　if (($load_now == 1)); then 
　　　　echo "$IP服务器15分钟的系统平均负载达到 $average_load，超过警戒值0.70，请及时处理。" | mailx -v -s "$IP 服务器系统负载告警" yinminxin@xxx.com
　　fi 
fi
...
if (($cpu_idle < 20)); then 
　　echo "$IP服务器cpu剩余$cpu_idle%，使用率已经超过80%，请及时处理。" | mailx -v -s "$IP 服务器CPU告警" yinminxin@xxx.com 
fi
